...
The clean canonical three-workload Preview consists of Prepare; Open/resolve unique durable base windows and round-robin place them; then bounded Close. Reset remains in Open/Place as an explicit fresh-demonstration policy rather than being mixed into DLL staging. The earlier successful pair remains unchanged under `workloads/dll-backed/regression/`. The canonical lifecycle is now runtime-proven in the recorded Desktop Connector environment. Office/Knowledge Worker and VDI validation remain later tracks.
