...
- **Office Preview:** Word, Excel, PowerPoint, and classic Outlook preflight stable process/class matches and use documented `START`. Edge requires zero existing durable windows, uses the proven `START(processName: "msedge")`/`MainWindow` path, independently requires exactly one durable Edge base window, and compares HWNDs before allocating. Word/Excel/PowerPoint have one-machine 6.8.6 runtime evidence; corrected Edge and classic Outlook remain runtime-pending. New Outlook is unsupported/unvalidated.
