...
1. **Static/build validation** — run `scripts/Test-Repository.ps1`, including unit/lock/state tests, source contracts, hashes, public safety, DLL dependency/framework/checksum checks, and source review. This proves pure logic/build only.
2. **Script Editor compile** — compile each individual script-only, DLL-backed, Office Preview, and Knowledge Worker workload.
...
12. **Actual sequential Login Enterprise test** — generic canonical execution, lifecycle, cleanup, and cross-file state are proven in the recorded environment.
